...
rules, recursive append-only rules, recursive descriptor-control and filesystem
socket connect operation rules, regular-file write opens, `write(2)`, `writev(2)`,
...
fs/namei.c, fs/open.c, fs/read_write.c, fs/attr.c, fs/xattr.c, fs/stat.c, fs/readdir.c, fs/fhandle.c, fs/namespace.c, net/unix/af_unix.c, security/security.c:
  ...
  denial, append-only, recursive split-operation tree, recursive descriptor and
  filesystem-socket operation tree, and recursive append-only tree rules by
  compiled identity
